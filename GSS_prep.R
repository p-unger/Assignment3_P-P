...
# M1 subset for college educated women
M1 <- lm(vhappy ~ career*married + age + agesq + as.factor(year) + as.factor(race) + as.factor(bdec), 
         data = subset(z, sex==2 & educat == 4))

### Check for full time workers & working_ft == 1
summary(M1)

require("interplot")

interM1 <- interplot(M1, var1 = "married", var2 = "career") +
